...
class SudokuSolver
  ...
  def find_row(row_number)
    start = row_number * 9
    (start..start+8).map { |index| original_puzzle[index].to_i }
  end

  def find_column(column_number)
    (0..8).map { |index| original_puzzle[column_number + index*9].to_i }    
  end
  ...
  def find_square(square_index)
    ...
    (0..2).map do |line|
      (0..2).map do |index|
        square << original_puzzle[square_start + (index+line*9)].to_i
      end
    end
    square
  end
  ...
  def solve_puzzle
    original_puzzle.each_with_index do |value, index|
      if value.to_i == 0
        number = guess_a_number(index)
        original_puzzle[index] = number
      else
        original_puzzle[index] = value.to_i
      end
    end
  end
